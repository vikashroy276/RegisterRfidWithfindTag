...
public abstract class BaseActivity extends AppCompatActivity implements RFIDHandler.ResponseHandlerInterface {
    ...
    private void passTheTag(String tagData) {
       if (currentActivity != null && currentActivity instanceof RegistrationActivity) {
            ((RegistrationActivity) currentActivity).handleTagData(tagData);
        } else if (currentActivity != null && currentActivity instanceof IssuanceActivity) {
            ((IssuanceActivity) currentActivity).handleTagData(tagData);
        }else if (currentActivity != null && currentActivity instanceof BinningActivity) {
            ((BinningActivity) currentActivity).handleTagData(tagData);
       }
    }
